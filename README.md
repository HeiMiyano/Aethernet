...
A from-scratch replacement for **Mare Synchronos** — a Dalamud plugin and server stack that synchronizes Penumbra mods, Glamourer settings, and related character-customization plugins between paired FFXIV players.

> **Status: scaffold / pre-alpha.** The solution compiles and the architecture is in place, but it has not been tested against a live FFXIV client. See [`docs/FEATURE_PARITY.md`](docs/FEATURE_PARITY.md) for the Mare-feature checklist with current status.
...
|---|---|---|
| `Aethernet.Plugin` | Dalamud plugin (net8.0-windows) | The client — runs in FFXIV, hooks Penumbra/Glamourer IPC, talks to the server. |
| `Aethernet.Server` | ASP.NET Core 8 service | SignalR hub: pairing, presence, character-data push, groups, moderation. |
| `Aethernet.AuthService` | ASP.NET Core 8 service | Registration (Discord OAuth + secret-key), JWT issuance, refresh, recovery. |
| `Aethernet.FileServer` | ASP.NET Core 8 service | Content-addressed upload/download of mod blobs by SHA-1. |
...
Prerequisites: .NET 8 SDK, Docker, Dalamud dev environment for the plugin half.
...
- [`docs/FEATURE_PARITY.md`](docs/FEATURE_PARITY.md) — every Mare feature mapped to Aethernet status.
...
To be decided. Mare Synchronos is AGPL-3.0; if Aethernet borrows any structural ideas (it does), AGPL-3.0 is the safest choice.

## Disclaimer

Aethernet is unofficial, not affiliated with or endorsed by Square Enix. Mod synchronization is a sensitive area; use responsibly.
